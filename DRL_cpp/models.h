...
struct ActorCriticImpl : public torch::nn::Module 
{
    ...
    ActorCriticImpl(int64_t n_in, int64_t n_out, double std)
        : // Actor. input,64,64,output (layers from ref. paper)
          a_lin1_(torch::nn::Linear(n_in, 64)),
          a_lin2_(torch::nn::Linear(64, 64)),
          a_lin3_(torch::nn::Linear(64, n_out)), 
          mu_(torch::full(n_out, 0.)),  // stochastic policy -|__?
          log_std_(torch::full(n_out, std)), // stochastic policy  -|__?
          
          // Critic input,64,64,output (layers from ref. paper)
